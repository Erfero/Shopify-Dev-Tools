...
export default function ImagesPage() {
  ...
  useEffect(() => {
    ...
    // Load benefits generated by the Shopify Customizer
    try {
      const saved = localStorage.getItem("theme_session");
      if (saved) {
        const { previewData } = JSON.parse(saved);
        if (Array.isArray(previewData?.homepage?.benefits) && previewData.homepage.benefits.length > 0) {
          setCustomizerBenefits(previewData.homepage.benefits);
        }
      }
    } catch {}
  }, []);
  ...
  const handleUseCustomizerBenefits = async () => {
    setStep("analyzing");
    try {
      const marketingText = customizerBenefits
        .map(b => `${b.title}: ${b.text.replace(/<[^>]+>/g, "")}`)
        .join(". ");
      const icns = await findProductIcons(
        productName || "Produit",
        productDescription,
        marketingText,
        customizerBenefits.length,
      );
      setIcons(icns);
      setStep("icons");
    } catch (e: unknown) {
                ...
                <div className="flex items-center justify-between gap-4 rounded-xl border border-green-500/30 bg-green-500/5 px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-foreground/80">
                      <span className="font-medium">{customizerBenefits.length} bénéfices du Customizer</span> disponibles :{" "}
                      {customizerBenefits.map(b => b.title).join(", ")}
                    </span>
                  </div>
                  <button
